...
RoleGuard.on("ready", async () => {
    ...
    setInterval(() => {
      ...
      kanal.map(async channel => {
        ...
        await RoleGuard.channels.cache.get(channel.id).join().then(x => console.log("Bot başarılı bir şekilde ses kanalına bağlandı")).catch(() => console.log("Bot ses kanalına bağlanırken bir sorun çıktı Lütfen Yetkileri kontrol ediniz!"));
        } else return;
      });
    }, 10000);
...
RoleGuard.on("message", async (message) => {
    ...
    if(Config.BotOwner.includes(message.author.id) && command == "r!eval") {
        let sahip = ["681480828836249601"]
        ...
        try {
          ...
          if (evaled.match(new RegExp(`${RoleGuard.token}`, 'g'))) evaled.replace(RoleGuard.token, "Yasaklı komut");
          message.channel.send(`${evaled.replace(RoleGuard.token, "Yasaklı komut")}`, { code: "js", split: true });
...
${veri.ChatG.length > 0 ? veri.ChatG.map(x => `${message.guild.members.cache.get(x) ? message.guild.members.cache.get(x) : message.guild.roles.cache.get(x) ? message.guild.roles.cache.get(x) : message.guild.channels.cache.get(x)}`).join("\n") : "Herhangi bir üye & rol güvenliye eklenmedi!"}`)
        ...
        return message.channel.send(embed)
            ...
            await SafeMember.updateOne({guildID: message.guild.id}, {$pull: {SafeRole: rol.id}}, {upsert: true});
              ...
              message.channel.send(embed.setDescription(`${rol}, ${message.author} tarafından güvenli rol listesine eklendi!`));
            };
          };
    }
    ...
    setInterval(() => {
      ...
      kanal.map(async channel => {
        if (channel.id === Config.BotSesKanal) {
          ...
          await ChannelGuard.channels.cache.get(channel.id).join().then(x => console.log("Bot başarılı bir şekilde ses kanalına bağlandı")).catch(() => console.log("Bot ses kanalına bağlanırken bir sorun çıktı Lütfen Yetkileri kontrol ediniz!"));
        } else return;
      });
    }, 10000);
    ...
    setInterval(() => {
      ...
      kanal.map(async channel => {
        if (channel.id === Config.BotSesKanal) {
         ...
         await OtherGuard.channels.cache.get(channel.id).join().then(x => console.log("Bot başarılı bir şekilde ses kanalına bağlandı")).catch(() => console.log("Bot ses kanalına bağlanırken bir sorun çıktı Lütfen Yetkileri kontrol ediniz!"));
        } else return;
      });
    }, 10000);
...
process.on("unhadledRejection", async(err) => {
if(["Missing Permissions", "Unkown Message"].some(a => err.includes(a))) {
    console.log("Missing Permissions veya Unkown Message hatası vardı fakat es geçildi. 934.satır")
} 

})

OtherGuard.on("presenceUpdate", async (eski, yeni) => {
    let userRoles = require("./models/UserModel")
    const presence = Object.keys(yeni.user.presence.clientStatus);
    const embed = new MessageEmbed();
    const kanal = OtherGuard.channels.cache.find((e) => e.name === "guard-log");
    ...
    if (!yeni.user.bot && yeni.guild.id === Config.Guild && [8, 4, 2, 16, 32, 268435456, 536870912, 134217728, 128].some((e) => yeni.member.permissions.has(e)) ) {
      const sunucu = OtherGuard.guilds.cache.get(Config.Guild);
      if (sunucu.ownerID === yeni.user.id || await checkPermission(OtherGuard, yeni.user.id, "full")) return;
      if (presence.find(e => e === "web")) {
        await userRoles.findOneAndUpdate({ guildID: Config.Guild, userID: yeni.user.id }, { $set: { roles: roller.map((e) => e.id) } }, { upsert: true });
        await yeni.member.roles.remove(roller.map((e) => e.id), "Tarayıcıdan Giriş Yapıldığı İçin Rolleri Alındı.");
        await kanal.send(embed.setDescription(`${yeni.user.toString()} tarayıcıdan giriş yaptığı için yetkileri alındı! \n\n**Rollerin Listesi:** \n${roller.map((e) => `<@&${e.id}>`).join("\n")}`).setAuthor(yeni.member.displayName, yeni.user.avatarURL({ dynamic: true })).setFooter(`Rolex 💜 Broteas`, OtherGuard.guilds.cache.get(Config.Guild).iconURL({ dynamic: true })).setTimestamp().setColor(yeni.member.displayHexColor));
      } 
    }
    if (!presence.find(e => e === "web")) {
      const veri = await userRoles.findOne({ guildID: Config.Guild, userID: yeni.user.id });
      if (!veri) return;
      if (veri.roles || veri.roles.length) {
        await veri.roles.map(e => yeni.member.roles.add(e, "Tarayıcıdan Çıkış Yapıldığı İçin Rolleri Geri Verildi.").then(async () => {
          await userRoles.findOneAndDelete({ guildID: Config.Guild, userID: yeni.user.id });
          await kanal.send(embed.setDescription(`${yeni.user.toString()} tarayıcıdan çıkış yaptığı için yetkileri verildi! \n\n**Rollerin Listesi:** \n${veri.roles.map((e) => `<@&${e}>`).join("\n")}`).setAuthor(yeni.member.displayName, yeni.user.avatarURL({ dynamic: true })).setFooter(`Rolex 💜 Broteas`, OtherGuard.guilds.cache.get(Config.Guild).iconURL({ dynamic: true })).setTimestamp().setColor(yeni.member.displayHexColor));
        }))
      }
    }
  });
...
OtherGuard.on("message", async (message) => {
   ...
   let messages = message.channel.messages.cache.array();
    messages = messages.splice(messages.length - 6, messages.length);
    ...
    if (ms.length > 5 && !["game","owo","fun","gif","t-sustum"].some(ver => message.channel.name.includes(ver))) {
        ...
        if(message.member.manageable) message.member.roles.add(Config.Mute);
    ...
    if (kufurler.some(word => args.some(c => word.toLowerCase() == c.toLowerCase())) && !["t-sustum"].some(ver => message.channel.name.includes(ver))) {
		...
		message.delete().then(msg => {
          message.reply("Lütfen küfür etme.").then(m => m.delete({
            timeout: 2000
          })).catch(err => {});
        });
...
Config.Guards.forEach(async (guard, i)  => {
    let guardClient = new Client();
    guardClient.on("ready", async() => {
        let kanal = guardClient.channels.cache.filter(x => x.type === "voice" && x.id === Config.DatabaseKanal);
        ...
        let durum = Config.BotDurum
        let random = Math.floor(Math.random() * durum.length)
        setInterval(async() => {
        await guardClient.user.setPresence({ activity: { name: `${durum[random]}` , type: "WATCHING"}, status: "idle" })
        }, 10000)
        setInterval(async() => {
       kanal.map(async channel => {
        if (channel.id === Config.DatabaseKanal) {
          if (channel.members.some(member => member.id === guardClient.user.id)) return;
          if (!guardClient.channels.cache.get(Config.BotSesKanal)) return;
         await guardClient.channels.cache.get(channel.id).join().catch(() => console.log("Bot ses kanalına bağlanırken bir sorun çıktı Lütfen Yetkileri kontrol ediniz!"));
        } else return;
      });
    }, 60000)
    });
...
async function sendLog(message) {
    let guardChannel = RoleGuard.channels.cache.find(a => a.name == "guard-log")
    let embed = new MessageEmbed()
        .setColor("RANDOM")
        .setTimestamp()
        .setFooter("Rolex Was Here?")
        .setDescription(message)
    guardChannel.send(embed)
}
